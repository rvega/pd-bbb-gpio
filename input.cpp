...
void input_start_digital_loop(t_input* x){
   pthread_attr_t attr;
   ...
   if(pthread_create(&x->digital_loop, &attr, &input_digital_loop, x)){
      error("%s/input: Cannot poll digital inputs state.", LIBRARY_NAME);
      return;
   }
   x->digital_loop_running = 1;

   debug("Started digital loop", 0);
}

void input_stop_digital_loop(t_input* x){
   while(pthread_cancel(x->digital_loop)){
...
void input_init_digital_pin(t_input* x, float pin_num){
   ...
   if(x->digital_loop_running==0){
      input_start_digital_loop(x); 
   }
}
...
void* input_analog_loop(void* user_param){
   UNUSED_PARAMETER(user_param);
   //t_input* x = (t_input*)user_param;
...
void input_start_analog_loop(t_input* x){
   pthread_attr_t attr;
   if(pthread_attr_init(&attr)){
      error("%s/input: Cannot poll analog inputs state.", LIBRARY_NAME);
      return;
   }
   if(pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_DETACHED)){
      error("%s/input: Cannot poll analog inputs state.", LIBRARY_NAME);
   ...
   if(pthread_create(&x->analog_loop, &attr, &input_analog_loop, x)){
      error("%s/input: Cannot poll analog inputs state.", LIBRARY_NAME);
      return;
   }
   x->analog_loop_running = 1;

   debug("Started analog loop", 0);
}

void input_stop_analog_loop(t_input* x){
   while(pthread_cancel(x->analog_loop)){
      debug("Closing analog thread", 0);
   }
   debug("Closed analog thread", 0);
}
...
void *input_new(void) {
   ...
   x->digital_loop_running = 0;
   x->inited_digital_pins_count = 0;

   x->analog_loop_running = 0;
   x->inited_analog_pins_count = 0;
